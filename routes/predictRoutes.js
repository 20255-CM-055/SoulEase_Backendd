...
// 👉 Use env variable (better for production)
const AI_BASE_URL = process.env.AI_BASE_URL || "https://ey200506-ai-backend-18th.hf.space";

// ==============================
// 🔹 TEXT → EMOTION (PREDICT)
// ==============================
router.post("/", async (req, res) => {
  try {
    const { text, userId } = req.body;

    // ✅ validation (safe, won’t break existing)
    if (!text) {
      return res.status(400).json({ error: "Text is required" });
    }

    // 🔥 Call Hugging Face AI backend
    const response = await axios.post(`${AI_BASE_URL}/predict`, {
      text,
      userId
    });

    // ✅ send response back (no change in structure)
...
router.post("/speech-to-text", async (req, res) => {
  try {
    // ⚠️ NOTE:
    // This assumes you're already handling file upload (multer or similar)

    const formData = req.body;

    const response = await axios.post(
      `${AI_BASE_URL}/speech-to-text`,
      formData,
      {
        headers: {
